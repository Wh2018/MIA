"""Evaluate CPsyCounD predictions with an LLM-as-a-judge protocol.
...
DEFAULT_DIMS = tuple(SCORE_RANGES)
...
def parse_context(prompt: str) -> str:
    ...
    for match in TAG_RE.finditer(user_payload):
        factors[match.group("tag")] = " ".join(match.group("body").split())
    ...
    lines = []
    if utterance:
        lines.append(f"Current client utterance: {utterance}")
...
def load_samples(prediction_file: Path, *, limit: int | None = None, start_index: int = 0) -> list[Sample]:
    ...
    with prediction_file.open("r", encoding="utf-8") as f:
        for raw_index, line in enumerate(f):
            ...
            samples.append(
                Sample(
                    index=int(obj.get("index", raw_index)),
                    prompt=prompt,
...
def parse_judge_scores(text: str, dimensions: list[str]) -> dict[str, int]:
    ...
    if not isinstance(data, dict):
        raise ValueError(f"judge response must be a JSON object: {text[:300]}")

    data_by_lower_key = {str(key).lower(): value for key, value in data.items()}
    scores: dict[str, int] = {}
    for dim in dimensions:
        if dim not in data_by_lower_key:
            raise ValueError(f"judge response missing {dim}: {text[:300]}")
        score = int(data_by_lower_key[dim])
        min_score, max_score = SCORE_RANGES[dim]
        if not min_score <= score <= max_score:
            raise ValueError(f"judge score for {dim} out of range {min_score}..{max_score}: {score}")
        scores[dim] = score
    return scores
...
def summarize(results_file: Path, summary_file: Path, dimensions: list[str]) -> dict[str, Any]:
    ...
    for dim_scores in per_sample.values():
        if all(dim in dim_scores for dim in dimensions):
            ...
            complete_normalized_avgs.append(
                mean(dim_scores[dim] / SCORE_RANGES[dim][1] for dim in dimensions)
            )
    ...
    normalized_average_scores = {
        dim: (mean(scores) / SCORE_RANGES[dim][1] if scores else None)
        for dim, scores in by_dim.items()
    }
    valid_dimension_means = [v for v in average_scores.values() if v is not None]
    valid_normalized_means = [v for v in normalized_average_scores.values() if v is not None]

    summary = {
        "num_records": sum(len(v) for v in by_dim.values()),
        "num_complete_samples": len(complete_avgs),
        "num_errors": errors,
        "score_ranges": {dim: {"min": SCORE_RANGES[dim][0], "max": SCORE_RANGES[dim][1]} for dim in dimensions},
        "average_scores": average_scores,
        "normalized_average_scores": normalized_average_scores,
        "overall_average": mean(valid_dimension_means) if valid_dimension_means else None,
        "overall_normalized_average": mean(valid_normalized_means) if valid_normalized_means else None,
        "overall_average_of_complete_samples": mean(complete_avgs) if complete_avgs else None,
        "overall_normalized_average_of_complete_samples": (
            mean(complete_normalized_avgs) if complete_normalized_avgs else None
        ),
        "dimensions": {
            dim: {
                "count": len(scores),
                "mean": mean(scores) if scores else None,
                "normalized_mean": mean(scores) / SCORE_RANGES[dim][1] if scores else None,
                "score_histogram": {
                    str(i): scores.count(i)
                    for i in range(SCORE_RANGES[dim][0], SCORE_RANGES[dim][1] + 1)
                },
            }
            for dim, scores in by_dim.items()
        },
...
def main() -> None:
    ...
    ap.add_argument(
        "--input-dir",
        default="data/training/predict/cpsy/MIA",
    ...
    ap.add_argument(
        "--prompt-dir",
        default="config/evaluation_prompts/cpsy",
        help="Directory containing all_dimensions.md.",
    )
    ap.add_argument(
        "--output-dir",
        default=None,
        help="Output directory. Default: <input-dir>/llm_judge/<model_eval>.",
    )
    ...
    system = (
        "You are a strict but fair evaluator for psychological counseling conversations. "
